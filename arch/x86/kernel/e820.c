...
// TODO: add explanation about this
struct e820_range {
	u64 start;
	u64 end;
	u64 entry_start;
...
enum e820_range_cover_type {
	E820_RANGE_NO_COVER,
	E820_RANGE_COVER_BY_RANGE,
	E820_RANGE_COVER_BY_ENTRY,
	E820_RANGE_PARTIALLY_COVERED_RANGE_FIRST,
	E820_RANGE_PARTIALLY_COVERED_ENTRY_FIRST,
	// TODO: Do I really want this?
	E820_RANGE_PARTIALLY_COVERED_SAME_START,
};
...
__e820__range_get_cover_type(const struct e820_range *range, u64 *update_size)
{
	if ((range->start <= range->entry_start) &&
		...
		*update_size += range->inner_end - range->inner_start;
		if (range->start == range->entry_start) {
			return E820_RANGE_PARTIALLY_COVERED_SAME_START;
		} else if (range->entry_start < range->start) {
			return E820_RANGE_PARTIALLY_COVERED_ENTRY_FIRST;
		} else { /* Else: range->entry_start > range->start */
			return E820_RANGE_PARTIALLY_COVERED_RANGE_FIRST;
		}
	} else {
...
// TODO: i need to be defined before, check if cad do better
#define FOR_EACH_E820_RANGE(table, entry, i)	\
	for (i = 0, entry = &table->entries[i]; \
	     i < table->nr_entries; \
...
__e820__range_update(struct e820_table *table, u64 start, u64 size, enum e820_type old_type, enum e820_type new_type, u8 crypto_capable)
{
	...
	FOR_EACH_E820_RANGE(table, entry, i) {
		...
		case E820_RANGE_COVER_BY_ENTRY:
			...
			break;
		case E820_RANGE_PARTIALLY_COVERED_RANGE_FIRST:
		case E820_RANGE_PARTIALLY_COVERED_SAME_START:
			pr_info("PART_RANGE_FIRST\n");
			...
			break;
		case E820_RANGE_PARTIALLY_COVERED_ENTRY_FIRST:
			pr_info("PART_ENTRY_FIRST\n");
			entry->size -= range.inner_end - range.inner_start;
			__e820__range_add(table, range.inner_start,
					  range.inner_end - range.inner_start,
					  new_type, entry->crypto_capable);
					      ...
					      enum e820_crypto_capabilities crypto_capable)
{
	...
	for (i = 0; i < table->nr_entries; i++) {
		...
		case E820_RANGE_COVER_BY_ENTRY:
			entry->size = range.start - range.entry_start;
			__e820__range_add(table, range.start, size, type,
					  crypto_capable);
			__e820__range_add(table, range.end,
					  range.entry_end - range.end,
